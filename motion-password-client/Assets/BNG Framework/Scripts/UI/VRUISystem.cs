    ...
    public class VRUISystem : BaseInputModule {
        ...
        void init() {
            if(cameraCaster == null) {
                ...
                cameraCaster.GetComponent<Camera>().stereoTargetEye = StereoTargetEyeMask.Both; // Jinghuai: need to set targetEye not to be none, otherwise there might be weird ray cast behavior.
                //cameraCaster.GetComponent<Camera>().targetDisplay = 1; // And set the target display as display 2 (not the same as the VR cam)
